...
class PROISP_VCaching
{
    ...
    public function init()
    {
        ...
        if ($this->check_if_purgeable()) {
            ...
            } else if (isset($purge_cache) && $purge_cache == 'cdn' && check_admin_referer($this->plugin)) {
                if (get_option('permalink_structure') == '' && current_user_can('manage_options')) {
                    add_action('admin_notices', array($this, 'pretty_permalinks_message'));
                }
                if ($this->varnishIp == null) {
    ...
    public function create_custom_fields()
    {
        if (function_exists('add_meta_box')) {
            foreach ($this->postTypes as $postType) {
                add_meta_box($this->plugin, __('Performance Cache', 'proisp-vcache'), array($this, 'display_custom_fields'), $postType, 'side', 'high');
            }
        }
    }
    ...
    {
        $vc_custom_field_nonce = isset($_POST['vc-custom-fields_wpnonce']) ? sanitize_text_field($_POST['vc-custom-fields_wpnonce']) : false;

        if ($vc_custom_field_nonce || !wp_verify_nonce($vc_custom_field_nonce, 'vc-custom-fields')) {
            return;
        }
    ...
    public function display_custom_fields()
    {
        ...
        foreach ($this->customFields as $customField) {
            ...
            if ($output) {
                switch ($customField['type']) {
                    case "checkbox":
                    {
                        ...
                        <label class="screen-reader-text"
                               for="<?php echo esc_attr($this->prefix . $customField['name']); ?>"><?php echo esc_html($customField['title']); ?></label>
                        <p><input type="checkbox" name="<?php echo esc_attr($this->prefix . $customField['name']); ?>"
                                  id="<?php echo esc_attr($this->prefix . $customField['name']); ?>"
                                  value="<?php echo esc_attr($yesVal); ?>" <?php echo $checkedVal; ?> <?php echo $style; ?> />
                        </p>
                        <?php break;
                    }
                    default:
                    {
                        ...
                        ?> <p><input type="text" name="<?php echo esc_html($this->prefix . $customField['name']); ?>"
                                     id="<?php echo esc_attr($this->prefix . $customField['name']); ?>"
                                     value="<?php echo esc_attr($value); ?>"/></p> <?php
                        break;
                    }
                }
            } else {
                ...
                <p><input type="text" name="<?php echo esc_html($this->prefix . $customField['name']); ?>"
                          id="<?php echo esc_html($this->prefix . $customField['name']); ?>"
                          value="<?php echo esc_attr($value); ?>" disabled/></p>
            <?php }
            $default_ttl = get_option($this->prefix . 'ttl');
            if ($customField['description']) { ?>
                <p><?php echo sprintf(esc_html__($customField['description'], 'proisp-vcache'), $default_ttl); ?></p>
                <?php
            }
        }
    }
    ...
    {
        echo '<div id="message" class="error fade"><p><strong>' . sprintf(__('Performance cache works with domains which are hosted on %sone.com%s.', 'proisp-vcache'), '<a href="https://one.com/" target="_blank" rel="noopener noreferrer">', '</a>') . '</strong></p></div>';
    }
